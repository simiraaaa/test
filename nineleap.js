...
(function() {
    ...
    /**
     * @member      tm.social.Nineleap
     * @method      createRankingURL
     * 9leap 用の URL を生成
     */
    tm.social.Nineleap.createRankingURL = function(id, score, result) {
    ...
    tm.social.Nineleap.postRanking = function(score, result) {
        if (location.hostname == 'r.jsgames.jp') {
            ...
            location.replace( this.createRankingURL(id, score, result) );
        }
        else {
     ...
     * ランキング上位のデータを取得する
     */
    tm.social.Nineleap.getRankingData = function(callback) {
        tm.util.Ajax.loadJSONP(tm.social.Nineleap.createRankingDataURL(5), callback);
    };
